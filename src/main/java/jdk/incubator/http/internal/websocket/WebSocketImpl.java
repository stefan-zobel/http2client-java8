...
public final class WebSocketImpl implements WebSocket {
    ...
    private class ReceiveTask extends SequentialScheduler.CompleteRestartableTask {
        ...
        private void processPing() {
            ...
            ByteBuffer copy = (ByteBuffer) ByteBuffer.allocate(binaryData.remaining())
                    .put(binaryData)
                    .flip();
            // Non-exclusive send;
    ...
    private boolean tryChangeState(State expectedState, State newState) {
        State witness = AtomicRef.compareAndExchange(state, expectedState, newState);
    ...
    static final class AtomicRef {
       private static final sun.misc.Unsafe U = UnsafeAccess.unsafe;
       private static final long VALUE;

       static State compareAndExchange(AtomicReference<State> atomicState, State expectedState, State newState) {
          Object witness;
          // XXX
          do {
             witness = U.getObjectVolatile(atomicState, VALUE);
          } while (witness == expectedState && !U.compareAndSwapObject(atomicState, VALUE, witness, newState));
