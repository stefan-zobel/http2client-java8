 ...
 *
 * This has two different usage modes, one is adding a selected protocol into
 * the extensions, the other is removing all mention of ALPN and returning the
 * selected protocol. This dual mode does not make for the cleanest code but
 * removes the need to have duplicate nearly identical methods.
 *
 * If the selected protocol is set then this will be added. If the selected
 * protocol is null then ALPN will be parsed and removed.
 *
 * <p>
...
final class ALPNHackServerHelloExplorer {
    ...
    static byte[] addAlpnExtensionsToServerHello(byte[] source, String selectedAlpnProtocol) throws SSLException {
        ...
        try {
            ...
            // there may be more messages (cert etc), so we append them
     ...
     */
    static byte[] removeAlpnExtensionsFromServerHello(ByteBuffer source,
            final AtomicReference<String> selectedAlpnProtocol) throws SSLException {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        try {
    ...
    private static void exploreHandshake(ByteBuffer input, int recordLength,
            AtomicReference<String> selectedAlpnProtocol, ByteArrayOutputStream out) throws SSLException {
        // What is the handshake type?
        byte handshakeType = input.get();
        if (handshakeType != 0x02) { // 0x01: server_hello message
            throw new SSLHandshakeException("Expected \"server hello\" record");
        }
    ...
    private static void exploreServerHello(ByteBuffer input, AtomicReference<String> alpnProtocolReference,
    ...
    private static String exploreExtensions(ByteBuffer input, ByteArrayOutputStream extensionOut, boolean removeAlpn)
        ...
        while (length > 0) {
            // extension type
            int extType = ALPNHackUtils.getInt16(input);
            // length of extension data
            int extLen = ALPNHackUtils.getInt16(input);
            if (extType == 16) {
                int vlen = ALPNHackUtils.getInt16(input);
                ret = ALPNHackUtils.readByteVector8(input);
                if (!removeAlpn) {
        ...
        if (removeAlpn && ret == null) {
            // There was no ALPN to remove, so this whole thing is unnecessary.
