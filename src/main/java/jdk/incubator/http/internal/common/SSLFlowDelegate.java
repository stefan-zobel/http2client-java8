...
public class SSLFlowDelegate {
     ...
     */
    public SSLFlowDelegate(SSLEngineEx engine,
                           Executor exec,
   ...
   /**
    * Returns a {@code CompletableFuture<String>} which completes after
...
//            case NEED_UNWRAP_AGAIN:
            ...
            case NOT_HANDSHAKING:
                throw new InternalError("Unexpected handshake status:"
                        + r.handshakeStatus());
            // case NEED_UNWRAP_AGAIN needs to fall through
            // Hence no default clause here 
        }
    }
        ...
        while (true) {
            ...
            switch (sslResult.getStatus()) {
                ...
                case CLOSED:
                    ...
                    // fall through. There could be some remaining data in dst.
