...
/**
 * Super hacky class that allows the ServerHello message to be modified and the
 * corresponding hash generated at runtime.
 *
 * @author Stuart Douglas
 */
final class ALPNHackServerBAOS extends ByteArrayOutputStream {
    ...
    ALPNHackServerBAOS(SSLEngine sslEngine, byte[] bytes, String alpnProtocol) {
    ...
    public void write(byte[] b, int off, int len) {
        if (ready) {
            if (b[off] == 2) { // server hello
                ...
                SSLEngineEx.regenerateHashes(sslEngine, this, toByteArray(), serverHello);
                return;
            }
        }
