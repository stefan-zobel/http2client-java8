...
/**
 * This class is used to both read and write the ALPN protocol names in the
 * ClientHello SSL message.
 *
 * If the out parameter is not null then the read function is being used, while
 * if it is present then it is rewriting the hello message to include ALPN.
 *
 * Even though this dual approach is not particularly clean it does remove the
 * need to have two versions of each function, that do almost exactly the same
 * thing.
 */
final class ALPNHackClientHelloExplorer {
    ...
    static List<String> exploreClientHello(ByteBuffer source) throws SSLException {
        ...
        if ((firstByte & 0x80) != 0 && thirdByte == 0x01) {
            ...
            throw new SSLHandshakeException(SSLHandshakeException.NOT_HANDSHAKE_RECORD);
        }
    }
        ...
        try {
            ...
            if ((firstByte & 0x80) != 0 && thirdByte == 0x01) {
                ...
                if (secondByte == 3 && thirdByte == 3) {
                    // TLS1.2 is the only one we care about.
                    // Previous versions can't use HTTP/2, newer versions
                    // won't be backported to JDK8
     ...
     */
    //@formatter:on
    private static void exploreTLSRecord(ByteBuffer input, byte firstByte, byte secondByte, byte thirdByte,
            List<String> alpnProtocols, ByteArrayOutputStream out) throws SSLException {
        ...
        int recordLength = ALPNHackUtils.getInt16(input);
        ...
        try {
            ...
            throw new SSLHandshakeException("Initial SSL/TLS handshake record is invalid");
        }
    }
     ...
     */
    //@formatter:on
    private static void exploreHandshake(ByteBuffer input, byte recordMajorVersion, byte recordMinorVersion,
            int recordLength, List<String> alpnProtocols, ByteArrayOutputStream out) throws SSLException {
        // What is the handshake type?
        byte handshakeType = input.get();
        if (handshakeType != 0x01) { // 0x01: client_hello message
            throw new SSLHandshakeException("Expected \"client hello\" record");
        }
        if (out != null) {
        ...
        int handshakeLength = ALPNHackUtils.getInt24(input);
        if (out != null) {
            // placeholder
            out.write(0);
            out.write(0);
        ...
        if (handshakeLength > recordLength - 4) { // 4: handshake header size
            throw new SSLHandshakeException(SSLHandshakeException.MULTIRECORD_HANDSHAKE);
        }
     ...
     */
    //@formatter:on
    private static void exploreClientHello(ByteBuffer input, List<String> alpnProtocols, ByteArrayOutputStream out)
            throws SSLException {
        // client version
    ...
    private static void exploreExtensions(ByteBuffer input, List<String> alpnProtocols, ByteArrayOutputStream out)
        ...
        int length = ALPNHackUtils.getInt16(input); // length of extensions
        writeInt16(extensionOut, 0); // placeholder
        while (length > 0) {
            int extType = ALPNHackUtils.getInt16(input); // extension type
            writeInt16(extensionOut, extType);
            int extLen = ALPNHackUtils.getInt16(input); // length of extension data
    ...
    private static void exploreALPNExt(ByteBuffer input, List<String> alpnProtocols) {
        int length = ALPNHackUtils.getInt16(input);
        int end = input.position() + length;
        while (input.position() < end) {
            alpnProtocols.add(ALPNHackUtils.readByteVector8(input));
        }
    }

    private static void processByteVector8(ByteBuffer input, ByteArrayOutputStream out) {
        int int8 = ALPNHackUtils.getInt8(input);
        if (out != null) {
            out.write(int8 & 0xFF);
        }
    ...
    private static void processByteVector16(ByteBuffer input, ByteArrayOutputStream out) {
        int int16 = ALPNHackUtils.getInt16(input);
