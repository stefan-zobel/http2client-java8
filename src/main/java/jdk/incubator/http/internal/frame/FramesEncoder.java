...
public class FramesEncoder {
    ...
    private List<ByteBuffer> joinWithPadding(ByteBuffer buf, List<ByteBuffer> data, int padLength) {
        int len = data.size();
        if (len == 0) return Lists.of(buf, getPadding(padLength));
        else if (len == 1) return Lists.of(buf, data.get(0), getPadding(padLength));
        else if (len == 2) return Lists.of(buf, data.get(0), data.get(1), getPadding(padLength));
    ...
    private List<ByteBuffer> join(ByteBuffer buf, List<ByteBuffer> data) {
        int len = data.size();
        if (len == 0) return Lists.of(buf);
        else if (len == 1) return Lists.of(buf, data.get(0));
        else if (len == 2) return Lists.of(buf, data.get(0), data.get(1));
