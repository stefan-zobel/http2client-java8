...
public final class InputStreams {
     ...
     * method blocks until the requested number of bytes have been read, end of
     * stream is detected, or an exception is thrown. This method does not close
     * the input stream.
     *
     * <p>
     * The length of the returned array equals the number of bytes read from the
     * stream. If {@code len} is zero, then no bytes are read and an empty byte
     * array is returned. Otherwise, up to {@code len} bytes are read from the
     * stream. Fewer than {@code len} bytes may be read if end of stream is
     * encountered.
     *
     * <p>
     ...
     * to read the specified number of bytes into a byte array. The total amount
     * of memory allocated by this method is proportional to the number of bytes
     * read from the stream which is bounded by {@code len}. Therefore, the
     * method may be safely called with very large values of {@code len}
     ...
     * after some, but not all, bytes have been read. Consequently the input
     * stream may not be at end of stream and may be in an inconsistent state.
     * It is strongly recommended that the stream be promptly closed if an I/O
     * error occurs.
     *
     * @implNote The number of bytes allocated to read data from this stream and
     *           return the result is bounded by {@code 2*(long)len}, inclusive.
     *
     * @param len
     ...
     * <p>
     * Note that this method is intended for simple cases where it is convenient
     * to read all bytes into a byte array. It is not intended for reading input
     * streams with large amounts of data.
     *
     * <p>
     * The behavior for the case where the input stream is <i>asynchronously
     * closed</i>, or the thread interrupted during the read, is highly input
    ...
    public static int readNBytes(InputStream is, byte[] b, int off, int len) throws IOException {
        java9.util.Objects.checkFromIndexSize(off, len, b.length);
        int n = 0;
        while (n < len) {
