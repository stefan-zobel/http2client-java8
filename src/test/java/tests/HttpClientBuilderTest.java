...
import jdk.incubator.http.StaticProxySelector;
...
public class HttpClientBuilderTest {
    ...
    public void testDefaults() throws Exception {
        List<HttpClient> clients = Lists.of(HttpClient.newHttpClient(),
    ...
    public void testProxySelector() {
        HttpClient.Builder builder = HttpClient.newBuilder();
        ProxySelector a = StaticProxySelector.of(null);
        builder.proxy(a);
        assertTrue(builder.build().proxy().get() == a);
        ProxySelector b = StaticProxySelector.of(InetSocketAddress.createUnresolved("foo", 80));
        builder.proxy(b);
        assertTrue(builder.build().proxy().get() == b);
        assertThrows(NullPointerException.class, () -> builder.proxy(null));
        ProxySelector c = StaticProxySelector.of(InetSocketAddress.createUnresolved("bar", 80));
        builder.proxy(c);
...
//        b.setEnableRetransmissions(true); // XXX
        SSLParametersExAccess.setEnableRetransmissions(b, true); // XXX this works on Java >= 9 only!

        builder.sslParameters(b);
        assertTrue(builder.build().sslParameters() != b);
//        assertTrue(builder.build().sslParameters().getEnableRetransmissions()); // XXX
        // XXX this works on Java >= 9 only!
        assertTrue(SSLParametersExAccess.getEnableRetransmissions(builder.build().sslParameters()));
