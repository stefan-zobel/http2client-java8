...
public class RequestBuilderTest {
    ...
    public void testDefaults() {
        List<HttpRequest.Builder> builders = Lists.of(newBuilder().uri(uri),
                                                      newBuilder(uri));
    ...
    public void testHeaders() {
        ...
        List<HttpRequest> requests = Lists.of(
        ...
        for (HttpRequest r : requests) {
            ...
            assertEquals(r.headers().allValues("A"), Lists.of("B"));
            assertEquals(r.headers().allValues("C").size(), 0);
            assertEquals(r.headers().map().get("A"), Lists.of("B"));
            ...
            assertThrows(UOE, () -> r.headers().map().put("Z", Lists.of("Z")));
            ...
            assertThrows(UOE, () -> r.headers().allValues("A").addAll(Lists.of("Z")));
        ...
        for (HttpRequest r : requests) {
            ...
            assertEquals(r.headers().allValues("C"), Lists.of("D"));
            assertEquals(r.headers().map().get("C"), Lists.of("D"));
        ...
        for (HttpRequest r : requests) {
            assertEquals(r.headers().map().size(), 1);
            assertTrue(r.headers().firstValue("A").isPresent());
            assertTrue(r.headers().allValues("A").containsAll(Lists.of("B", "C")));
            assertEquals(r.headers().allValues("C").size(), 0);
            assertEquals(r.headers().map().get("A"), Lists.of("B", "C"));
            assertThrows(NFE, () -> r.headers().firstValueAsLong("A"));
            assertFalse(r.headers().firstValue("C").isPresent());
            // a non-exhaustive list of mutators
            assertThrows(UOE, () -> r.headers().map().put("Z", Lists.of("Z")));
    ...
    public void testCopy() {
        ...
        assertEquals(copyRequest.headers().map().get("A"), Lists.of("B"));
