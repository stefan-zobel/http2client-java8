...
import tests.http2.server.BodyOutputStream;
import tests.http2.server.Http2Handler;
import tests.http2.server.Http2TestExchange;
import tests.http2.server.Http2TestExchangeImpl;
import tests.http2.server.Http2TestServer;
import tests.http2.server.Http2TestServerConnection;
...
public class ContinuationFrameTest {
    ...
    static BiFunction<Integer,List<ByteBuffer>,List<Http2Frame>> oneContinuation =
        (Integer streamid, List<ByteBuffer> encodedHeaders) -> {
            List<ByteBuffer> empty =  java9.util.Lists.of(ByteBuffer.wrap(new byte[0]));
            ...
            ContinuationFrame cf = new ContinuationFrame(streamid,
                                                         ...
                                                         encodedHeaders);
            return java9.util.Lists.of(hf, cf);
        };
        ...
        (Integer streamid, List<ByteBuffer> encodedHeaders) -> {
            ...
            for (ByteBuffer bb : encodedHeaders) {
                while (bb.hasRemaining()) {
                    List<ByteBuffer> data = java9.util.Lists.of(ByteBuffer.wrap(new byte[] {bb.get()}));
