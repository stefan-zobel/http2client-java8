...
public class Timeout {
    ...
    private static final String KEYSTORE = getKeystorePath();
...
//                    params.setApplicationProtocols(new String[] {"h2"});
                    params = SSLParametersExAccess.init(params, new String[] {"h2"}); // XXX this works on Java >= 9 only!
    ...
    private static String getKeystorePath() {
        String ksPath = System.getProperty("test.src");
        if (ksPath != null) {
            ksPath += (File.separator + "keystore.jks");
        } else {
            ksPath = SameDirectory.getPackagePath(Timeout.class) + "/keystore.jks";
            System.err.println("Using fallback keystore path: " + ksPath);
        }
        return ksPath;
    }
}
