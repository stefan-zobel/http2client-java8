...
import com_.sun.net.httpserver.EchoHandler;
import com_.sun.net.httpserver.FileServerHandler;
...
public class SmokeTest {
    ...
    private static void realmain(String[] args) throws Exception {
        initServer();
        fileroot = System.getProperty ("test.src");
        if (fileroot != null) {
            fileroot += "/docs";
        } else {
            fileroot = SameDirectory.getPath(SmokeTest.class) + "docs";
            System.err.println("Using fallback fileroot path: " + fileroot);
        }
        ...
        try {
            ...
            test4(httpsroot + "files/foo.txt"); // XXX fails in SSLEngineEx.regenerateHashes!
...
*/
    ...
    static class CountingProxySelector extends ProxySelector {
        ...
        private CountingProxySelector(InetSocketAddress proxyAddress) {
            proxySelector = StaticProxySelector.of(proxyAddress);
        }
    ...
    static void initServer() throws Exception {
        ...
        String root = System.getProperty ("test.src");
        if (root != null) {
            root += "/docs";
        } else {
            root = SameDirectory.getPath(SmokeTest.class) + "docs";
            System.err.println("Using fallback /docs path: " + root);
        }
