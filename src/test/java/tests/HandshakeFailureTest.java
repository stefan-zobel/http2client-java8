...
public class HandshakeFailureTest {
    ...
    private static void realmain(String[] args) throws Exception {
        ...
        List<AbstractServer> servers = Lists.of(new PlainServer(), new SSLServer());

        for (AbstractServer server : servers) {
            try (AbstractServer server2 = server) {
                out.format("%n%n------ Testing with server:%s ------%n", server2);
                URI uri = new URI("https://127.0.0.1:" + server2.getPort() + "/");
    ...
    private void testSyncSameClient(URI uri, Version version) throws Exception {
        ...
        for (int i = 0; i < TIMES; i++) {
            ...
            } catch (SSLException expOnJava8) {
                out.printf("Client: caught expected Java 8 exception: %s%n", expOnJava8);
            }
        }
    }

    private void testSyncDiffClient(URI uri, Version version) throws Exception {
    ...
    private void testAsyncSameClient(URI uri, Version version) throws Exception {
        ...
        for (int i = 0; i < TIMES; i++) {
            ...
            try {
                ...
                } else if (ce.getCause() instanceof SSLException) {
                    out.printf("Client: caught expected Java 8 exception: %s%n", ce.getCause());
                } else {
    ...
    private void testAsyncDiffClient(URI uri, Version version) throws Exception {
