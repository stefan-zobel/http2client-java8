 ...
 * for receiving unusual packet sizes. Additionally, tests scenarios where
...
public class SplitResponse {
        ...
        for (Version version : Version.values()) {
            for (boolean serverKeepalive : new boolean[]{ true, false }) {
                ...
                for (boolean async : new boolean[]{ true, false }) {
                    sp.test(version, serverKeepalive, async, useSSL);
                }
            }
        }
    }
    ...
    // @Test
    void test(Version version, boolean serverKeepalive, boolean async, boolean useSSL)
        throws Exception
    {
        out.println(format("*** version %s, serverKeepAlive: %s, async: %s ***",
                           version, serverKeepalive, async));
        MockServer server = new MockServer(0, factory, useSSL);
