...
public class AbstractSSLTubeTest extends AbstractRandomTest {
    ...
    protected void run(FlowTube server,
        ...
        for (long i = 0; i < COUNTER; i++) {
            ...
            count += LONGS_PER_BUF;
            p.submit(java9.util.Lists.of(b));
        }
    ...
    protected static SSLEngineEx createSSLEngine(boolean client) throws IOException {
        SSLContext context = (new SimpleSSLContext()).get();
        SSLEngine engine = context.createSSLEngine();
        SSLEngineEx engineX = new SSLEngineEx(engine);
        SSLParameters params = context.getSupportedSSLParameters();
        params.setProtocols(new String[]{"TLSv1.2"}); // TODO: This is essential. Needs to be protocol impl
        SSLParametersEx paramsX = new SSLParametersEx(params);
        if (client) {
            String[] appProtocols = new String[] {"proto1", "proto2"}; // server will choose proto2
            paramsX.setApplicationProtocols(appProtocols); // XXX this works on Java >= 9 only!
            engineX.setApplicationProtocols(appProtocols);
        } else {
            String[] appProtocols = new String[] {"proto2"}; // server will choose proto2
            paramsX.setApplicationProtocols(appProtocols); // XXX this works on Java >= 9 only!
            engineX.setApplicationProtocols(appProtocols);
        }
        engineX.setSSLParameters(paramsX.getDelegate());
        engineX.setUseClientMode(client);
        return engineX;
    }
}
